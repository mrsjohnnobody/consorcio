...
class PrestacaoContasController {
  async viewPrestacaoContas(req, res) {
    try {
        ...
        res.render("admin/prestacaoContas/prestacaoContasPage", { prestacaoContas: prestacaoContas, parlamentares: parlamentares});
    } catch (error) {
  ...
  async deletePrestacaoContas(req, res) {
    try{
      ...
      const prestacaoContas = await PrestacaoContas.findByPk(req.params.id);
